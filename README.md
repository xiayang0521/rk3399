...
ssh登陆后，以AM40原厂Android_mmcblk2备份为例, 使用 dd 和 gzip 备份，可以执行命令：
```
...
在还原时，将Android_am40_mmcblk2.img.gz上传到/root/, 可以执行下列命令：
```
gzip -dc Android_am40_mmcblk2.img.gz | dd of=/dev/mmcblk2
```

### 3. 把Armbian装进emmc
ssh登陆后，将Armbian_23.02.2_am40_jammy_current_6.1.11.img上传到/root/,执行
```
